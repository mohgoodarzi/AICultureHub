...
public class AdminService : IAdminService
{
    ...
    public async Task<RoleDto?> UpdateRoleAsync(int id, UpdateRoleRequest request, int modifiedBy) { var role = await _context.Roles.FindAsync(id); if (role == null) return null; if (request.Name != null) role.Name = request.Name; if (request.Description != null) role.Description = request.Description; if (request.IsActive.HasValue) role.IsActive = request.IsActive.Value; role.ModifiedDate = DateTime.UtcNow; await _context.SaveChangesAsync(); if (request.PermissionIds != null) { var viewCodes = new[] { "Dashboard.View", "Articles.View", "Courses.View", "Scoreboard.View", "Management.View", "AiPolicy.View" }; var validIds = await _context.Permissions.Where(p => p.IsActive && viewCodes.Contains(p.Code)).Select(p => p.Id).ToListAsync(); var filteredIds = request.PermissionIds.Where(pid => validIds.Contains(pid)).Distinct().ToList(); var existingPerms = await _context.RolePermissions.Where(rp => rp.RoleId == id).ToListAsync(); _context.RolePermissions.RemoveRange(existingPerms); _context.RolePermissions.AddRange(filteredIds.Select(pid => new RolePermission { RoleId = id, PermissionId = pid })); await _context.SaveChangesAsync(); } await _auditService.LogAsync(modifiedBy, "UPDATE_ROLE", "Role", id, $"Updated role: {role.Name}"); return await GetRoleByIdAsync(id); }
    ...
    private static string GetModuleDisplayName(string module) => module switch { "Dashboard" => "داشبورد", "Users" => "کاربران", "Roles" => "نقش‌ها", "Articles" => "مقالات", "Categories" => "دسته‌بندی", "Courses" => "دوره‌ها", "Quizzes" => "آزمون‌ها", "Challenges" => "چالش‌ها", "Announcements" => "اطلاعیه‌ها", "Reports" => "گزارشات", "Settings" => "تنظیمات", "Glossary" => "واژگان", "Scoreboard" => "جدول امتیازات", "Feedback" => "بازخورد", "AIPolicy" => "هوش مصنوعی", "Management" => "پنل مدیریت", _ => module };
    public async Task<bool> SetRolePermissionsAsync(int roleId, List<int> permissionIds, int modifiedBy) { var role = await _context.Roles.FindAsync(roleId); if (role == null) return false; var viewCodes = new[] { "Dashboard.View", "Articles.View", "Courses.View", "Scoreboard.View", "Management.View", "AiPolicy.View" }; var validIds = await _context.Permissions.Where(p => p.IsActive && viewCodes.Contains(p.Code)).Select(p => p.Id).ToListAsync(); var filteredIds = permissionIds.Where(pid => validIds.Contains(pid)).Distinct().ToList(); var existingPerms = await _context.RolePermissions.Where(rp => rp.RoleId == roleId).ToListAsync(); _context.RolePermissions.RemoveRange(existingPerms); _context.RolePermissions.AddRange(filteredIds.Select(pid => new RolePermission { RoleId = roleId, PermissionId = pid })); await _context.SaveChangesAsync(); await _auditService.LogAsync(modifiedBy, "SET_PERMISSIONS", "Role", roleId, $"Updated permissions for role: {role.Name}"); return true; }
