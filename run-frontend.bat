...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Invoke-WebRequest -Uri 'http://localhost:8080/' -UseBasicParsing -TimeoutSec 3; Write-Host 'Backend already running.' } catch { Write-Host 'Starting backend...'; Start-Process -FilePath 'dotnet' -ArgumentList 'run','--no-build' -WorkingDirectory 'D:\Ai Site\Backend\API' -WindowStyle Hidden; Start-Sleep -Seconds 14; try { Invoke-WebRequest -Uri 'http://localhost:8080/' -UseBasicParsing -TimeoutSec 5 | Out-Null; Write-Host 'Backend started.' } catch { Write-Host 'Backend did not start - run run-backend.bat' } }"
